...
class CProgress : public IAMCopyCaptureFileProgress
{
    ...
    {
        TCHAR tach[80];
        HRESULT hr = swprintf(tach, 80, TEXT("Save File Progress: %d%%\0"), i);
        ...
        case WM_FGNOTIFY:
            ...
            if(gcap.pME)
            {
                ...
                if(bAbort)
                {
                        ...
                        TCHAR szError[100];
                        HRESULT hr = swprintf(szError, 100, TEXT("ERROR during capture, error code=%08x\0"), hrAbort);
                        ErrMsg(szError);
                }
            }
            break;
...
void UpdateStatus(BOOL fAllStats)
{
    ...
    if(!fAllStats)
    {
        ...
        hr = swprintf(tach, 160, TEXT("Captured %d frames (%d dropped) %d.%dsec\0"), lNot,
                 lDropped, lTime / 1000,
    ...
    hr = swprintf(tach, 160, TEXT("Captured %d frames in %d.%d sec (%d dropped): %d.%d fps %d.%d Meg/sec\0"),
...
#define DESCSIZE 80
    ...
    if(gcap.pVC)
    {
        ...
        if(hr == S_OK)
        {
            ...
            {
                hr = swprintf(tachStatus, VERSIZE + DESCSIZE + 5, TEXT("%s - %s\0"), wachDesc, wachVer);
        ...
        case WM_COMMAND :
            switch(GET_WM_COMMAND_ID(wParam, lParam))
            {
                ...
                case IDD_SetCapFileSize:
                {
                    ...
                    if(bchanged)
                    {
                        HRESULT hr = swprintf(tachBuffer, 21, TEXT("%ld\0"), l);
...
int FAR PASCAL FrameRateProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            hr = swprintf(tach, 32, TEXT("%d\0"), (int) gcap.FrameRate);
...
int FAR PASCAL TimeLimitProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            hr = swprintf(tach, 32, TEXT("%d\0"), gcap.dwTimeLimit);
...
int FAR PASCAL PressAKeyProc(HWND hwnd, UINT msg, UINT wParam, LONG lParam)
{
    ...
    switch(msg)
    {
        case WM_INITDIALOG:
            ...
            hr = swprintf(tach, _MAX_PATH, TEXT("%s\0"), gcap.wszCaptureFile);
...
void OnClose()
{
    ...
    HRESULT hr = swprintf(szBuf, 512, TEXT("%d"), (int)(10000000 / gcap.FrameRate));
    WriteProfileString(TEXT("annie"), TEXT("FrameRate"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.fUseFrameRate);
    WriteProfileString(TEXT("annie"), TEXT("UseFrameRate"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.fCapAudio);
    WriteProfileString(TEXT("annie"), TEXT("CaptureAudio"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.fCapCC);
    WriteProfileString(TEXT("annie"), TEXT("CaptureCC"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.fWantPreview);
    WriteProfileString(TEXT("annie"), TEXT("WantPreview"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.iMasterStream);
    WriteProfileString(TEXT("annie"), TEXT("MasterStream"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.fUseTimeLimit);
    WriteProfileString(TEXT("annie"), TEXT("UseTimeLimit"), szBuf);

    hr = swprintf(szBuf, 512, TEXT("%d"), gcap.dwTimeLimit);
